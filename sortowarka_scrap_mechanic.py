...
def print_sorted():
    ...
    for folder in sorted_folders:
        ...
        readable_time = (
            datetime.fromtimestamp(latest_time).strftime("%Y-%m-%d %H:%M:%S")
            if latest_time
            else "No files"
        )
    ...
    for folder in folders:
        ...
        if folder_name != "0000Blueprints_work":
            ...
            try:
                ...
                print(f"Updated directory modification date at: {target_path}")
...
def move_back_to_main_folder():
    ...
    folders = [
        os.path.join(work_folder, folder)
        for folder in os.listdir(work_folder)
        if os.path.isdir(os.path.join(work_folder, folder))
    ]
    sorted_folders = sorted(folders, key=get_latest_mod_time, reverse=True)
